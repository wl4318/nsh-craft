...
self.addEventListener('fetch', function(e) {
  ...
  if (e.request.destination === 'document') {
    e.respondWith(fetch(e.request).then(function(r) {
      var rc = r.clone(); caches.open(CACHE).then(function(ca){ca.put(e.request, rc);}); return r;
    }).catch(function(){ return caches.match(e.request); }));
  } else {
    e.respondWith(caches.match(e.request).then(function(c) {
      return c || fetch(e.request).then(function(r) {
        if(r.ok){var rc=r.clone();caches.open(CACHE).then(function(ca){ca.put(e.request,rc)});} return r;
